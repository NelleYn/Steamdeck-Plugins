...
def _bundled_root(plugin_dir: Path | str) -> Path:
    ...
    return Path(plugin_dir) / "vendored" / "rclone"


def _bundled_binary(plugin_dir: Path | str) -> Path | None:
    root = _bundled_root(plugin_dir)
    if not root.exists():
        return None
    for subdir in root.iterdir():
        candidate = subdir / "rclone"
...
async def install(
    ...
    if plugin_dir is not None:
        ...
        if bundled is not None:
            dest_subdir = root / bundled.parent.name
            try:
                await asyncio.to_thread(
                    shutil.copytree, bundled.parent, dest_subdir, dirs_exist_ok=True,
                )
                (dest_subdir / "rclone").chmod(0o755)
            except Exception as exc:
                return {"ok": False, "error": f"bundled_copy:{exc}"}
            return {"ok": True, "path": str(dest_subdir / "rclone"), "source": "bundled"}
